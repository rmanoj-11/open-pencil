...
const FILL_CATEGORY: Record<string, FillCategory> = {
  SOLID: 'SOLID',
  GRADIENT_LINEAR: 'GRADIENT', GRADIENT_RADIAL: 'GRADIENT',
  GRADIENT_ANGULAR: 'GRADIENT', GRADIENT_DIAMOND: 'GRADIENT',
  IMAGE: 'IMAGE'
}
...
export function useFillPicker(fill: Fill, onUpdate: (fill: Fill) => void) {
  ...
  function toGradient() {
    ...
    const stops: GradientStop[] = fill.gradientStops?.length
      ? fill.gradientStops
      : [{ color: { ...fill.color }, position: 0 }, { color: { r: 1, g: 1, b: 1, a: 1 }, position: 1 }]
    onUpdate({ ...fill, type: 'GRADIENT_LINEAR', gradientStops: stops, gradientTransform: { m00: 1, m01: 0, m02: 0, m10: 0, m11: 0, m12: 0.5 } })
  }
  ...
  const swatchBg = useMemo(() => {
    if (category === 'GRADIENT' && fill.gradientStops?.length) return `linear-gradient(to right, ${gradientCSS(fill.gradientStops)})`
    return colorToCSS(fill.color)
  }, [category, fill])

  return { category, swatchBg, toSolid, toGradient, toImage }
}