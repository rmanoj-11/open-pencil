...
export function useGradientStops(fill: Fill, onUpdate: (fill: Fill) => void) {
  ...
  const barBackground = useMemo(() =>
    stops.length ? `linear-gradient(to right, ${stops.map((s) => `${colorToCSS(s.color)} ${s.position * 100}%`).join(', ')})` : '',
  [stops])
  ...
  function selectStop(index: number) { setActiveStopIndex(index) }
  ...
  return {
    activeStopIndex, stops, subtype, subtypes: SUBTYPES,
    activeColor, barBackground,
    setSubtype, selectStop, addStop, removeStop,
    updateStopPosition, updateStopColor, updateStopOpacity,
    updateActiveColor, dragStop
  }
}