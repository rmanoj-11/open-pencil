...
import type { Ref } from '#react/internal/ref-type'

import { colorToHexRaw, parseColor } from '@open-pencil/core/color'
import { BLACK } from '@open-pencil/core/constants'
import { randomHex } from '@open-pencil/core/random'
import type { Variable, VariableCollection, VariableType, VariableValue } from '@open-pencil/core/scene-graph'
...
export function createVariableCollectionActions(editor: Editor, activeCollectionId: Ref<string>) {
  ...
  function addCollection() {
    ...
    const collection: VariableCollection = {
      id, name: 'New collection', modes: [{ modeId: 'default', name: 'Mode 1' }],
      defaultModeId: 'default', variableIds: []
    }
  ...
  return { setActiveCollection, addCollection, renameCollection, removeCollection, addMode, removeMode, renameMode, setDefaultMode, duplicateMode, setActiveMode }
}

export function createVariableValueActions(editor: Editor, getActiveCollection: () => VariableCollection | null) {
  ...
  function addVariable(type: VariableType = 'COLOR') {
    ...
    editor.addVariable({ id, name: defaultVariableName(type), type, collectionId: col.id, valuesByMode, description: '', hiddenFromPublishing: false })
  }

  function removeVariable(id: string) { editor.removeVariable(id) }
  function renameVariable(id: string, newName: string) { editor.renameVariable(id, newName) }
  function updateVariableValue(id: string, modeId: string, value: VariableValue) { editor.updateVariableValue(id, modeId, value) }
  ...
  return { addVariable, removeVariable, renameVariable, updateVariableValue, formatModeValue, parseVariableValue, shortName }
}