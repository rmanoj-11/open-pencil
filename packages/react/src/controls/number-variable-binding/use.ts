...
export type NumberBindingPath =
  ...
  | 'opacity' | 'fontSize' | 'fontWeight' | 'lineHeight' | 'letterSpacing' | 'paragraphSpacing' | 'paragraphIndent'

export function useNumberVariableBinding(path: NumberBindingPath) {
  const binding = useVariableBinding({ type: 'FLOAT', path })
  ...
  return { ...binding, numberVariables: binding.variables, createAndBindVariable }
}