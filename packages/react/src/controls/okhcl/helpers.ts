import {
  getFillOkHCL, getStrokeOkHCL, resolveOkHCLForPreview,
  rgbaToOkHCL, setNodeFillOkHCL, setNodeStrokeOkHCL
...
export function createOkHCLActions(editor: Editor) {
  ...
  function updateFillOkHCL(node: SceneNode, index: number, patch: Partial<OkHCLColor>) {
    ...
    editor.updateNodeWithUndo(node.id, setNodeFillOkHCL(node, index, { ...current, ...patch }), 'Change fill OkHCL')
  }
  function updateStrokeOkHCL(node: SceneNode, index: number, patch: Partial<OkHCLColor>) {
    ...
    editor.updateNodeWithUndo(node.id, setNodeStrokeOkHCL(node, index, { ...current, ...patch }), 'Change stroke OkHCL')
  }
...
export function createOkHCLFieldFormats(
  fieldFormats: { value: Map<string, ColorFieldFormat> },
