...
export function createStrokeSideActions(editor: Editor, sideMenuOpen: { value: boolean }) {
  function selectSide(side: StrokeSides, activeNode: SceneNode) {
    ...
    if (side === 'ALL') {
      editor.updateNodeWithUndo(activeNode.id, {
        independentStrokeWeights: false, borderTopWeight: 0, borderRightWeight: 0, borderBottomWeight: 0, borderLeftWeight: 0
      } as Partial<SceneNode>, 'Stroke all sides')
    } else if (side === 'CUSTOM') {
      const w = activeNode.independentStrokeWeights
        ? { top: activeNode.borderTopWeight, right: activeNode.borderRightWeight, bottom: activeNode.borderBottomWeight, left: activeNode.borderLeftWeight }
        : { top: weight, right: weight, bottom: weight, left: weight }
      editor.updateNodeWithUndo(activeNode.id, {
        independentStrokeWeights: true, borderTopWeight: w.top, borderRightWeight: w.right, borderBottomWeight: w.bottom, borderLeftWeight: w.left
      } as Partial<SceneNode>, 'Custom stroke sides')
    } else {
      editor.updateNodeWithUndo(activeNode.id, {
      ...
      } as Partial<SceneNode>, `Stroke ${side.toLowerCase()} only`)
    }
