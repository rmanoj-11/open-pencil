...
export function useColorVariableBinding(kind: ColorBindingKind) {
  const binding = useVariableBinding({ type: 'COLOR', path: (index) => `${kind}/${index}/color` })
  ...
  function createAndBindVariable(nodeId: string, index: number, color: Color, name = FALLBACK_COLOR_VARIABLE_NAME) {
  ...
  return {
    ...
    bindVariable: (nodeId: string, index: number, variableId: string) => binding.bindVariable(nodeId, variableId, index),
