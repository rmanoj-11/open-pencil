...
export function createNodePropSelectionState(store: Editor) {
  function getNode(): SceneNode | null {
    void store.state.sceneVersion
    return store.getSelectedNode() ?? null
  }

  function getNodes(): SceneNode[] {
    void store.state.sceneVersion
  ...
  function active(): boolean {
    return getNode() !== null || isMulti()
  }

  function activeNode(): SceneNode | null {
    return getNode() ?? (getNodes()[0] as SceneNode | undefined) ?? null
  }
  ...
  return { getNode, getNodes, isMulti, active, activeNode, merged, prop, updateAllWithUndo }
}
...
export function createNodePropArrayActions({
  ...
  function targetNodes(): SceneNode[] {
    if (isMulti()) return getNodes()
    return getActiveNode() ? [getActiveNode()!] : []
  }
